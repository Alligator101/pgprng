// ---------------------------------------------------------------------
// Base file: PractRand 0.86 test/RNG_test.cpp (c) its original authors.
// Modifications (stdin RNG adapter, throttle removal) developed
// collaboratively by Aly Graham and Claude (Anthropic), September 2026.
// See SESSION_CHANGES_README.txt for what changed and why.
// ---------------------------------------------------------------------
#include <cstdio>
#include <cstdlib>
#include <cmath>
...
/*
Reads raw 64-bit little-endian words from stdin -- feeds an external
generator's output (piped in) directly into PractRand's test battery
instead of using one of PractRand's own built-in RNGs.
*/
class Raw_StdinRNG64 {
public:
	enum {
	...
	static Uint64 total_calls;
	Uint64 raw64() {
		Uint64 return_value;
		total_calls++;
		if (1 == std::fread(&return_value, sizeof(return_value), 1, stdin)) return return_value;
		std::fprintf(stderr, "stdin exhausted after %llu raw64 calls (%llu bytes)\n", (unsigned long long)total_calls, (unsigned long long)total_calls*8);
		std::exit(0);
	}
	void walk_state(StateWalkingObject *walker) {}
};
Uint64 Raw_StdinRNG64::total_calls = 0;
class Polymorphic_StdinRNG64 : public PractRand::RNGs::vRNG64 {
public:
	Raw_StdinRNG64 implementation;
	Uint64 raw64() {return implementation.raw64();}
	void walk_state(StateWalkingObject *walker) {implementation.walk_state(walker);}
	std::string get_name() const {return std::string("StdinRNG64(external)");}
};
...
void do_statistical_tests() {
	...
	RNGs::vRNG *rng = new Polymorphic_StdinRNG64();
	//no seed() call -- the "RNG" here is just a stdin reader, its
	//randomness comes entirely from whatever is piped in
...
//	Tests::ListOfTests tests = Tests::Batteries::get_expanded_standard_tests(rng);
	...
	while (test_size <= 1ull<<48) {
		...
		if (time_passed < 0.0) continue;  // patched: always print (was 1.6s throttle)
